...
celery_schedule = {  # 任务的周期调度队列
    'refresh_queries': {
        'task': 'redash.tasks.refresh_queries',
        'schedule': timedelta(seconds=30)  # 调度周期(每30秒)
    },
    'cleanup_tasks': {
...
if settings.VERSION_CHECK:
    celery_schedule['version_check'] = {
        ...
        # evenly.
        'schedule': crontab(minute=randint(0, 59), hour=randint(0, 23))  # 每天中随即一个时刻
    }
...
# 更新这个celery对象的配置信息
celery.conf.update(CELERY_RESULT_BACKEND=settings.CELERY_BACKEND,  # 消息结果存储

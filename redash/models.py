# 该文件创建了整个redash的数据model，之后会用这个数据model进行建表
import cStringIO
import csv
import datetime
import functools
...
class DataSource(BelongsToOrgMixin, db.Model):
    ...
    name = Column(db.String(255))  # 数据源名字： "运营数据ES"
    type = Column(db.String(255))  # 数据源类型：  elasticsearch
    options = Column(ConfigurationContainer.as_mutable(Configuration))  # {"server": "http://10.0.0.227:9200"}
    queue_name = Column(db.String(255), default="queries")
    scheduled_queue_name = Column(db.String(255), default="scheduled_queries")
    created_at = Column(db.DateTime(True), default=db.func.now())  # 创建时间
...
class AccessPermission(GFKBase, db.Model):
    # 用户对dashboard或者query的修改权限，但是我想在准备把这个表改成查看权限使用
    id = Column(db.Integer, primary_key=True)
    # 'object' defined in GFKBase
    access_type = Column(db.String(255))  # 访问权限

    grantor_id = Column(db.Integer, db.ForeignKey("users.id"))  # 授权人ID
    grantor = db.relationship(User, backref='grantor', foreign_keys=[grantor_id])

    grantee_id = Column(db.Integer, db.ForeignKey("users.id"))  # 被授权人ID
...
class Dashboard(ChangeTrackingMixin, TimestampMixin, BelongsToOrgMixin, db.Model):
    ...
    def all(cls, org, group_ids, user_id):
        ...
        can_view_dashboard = [i.object_id for i in AccessPermission.query.filter(
            AccessPermission.grantee_id == user_id,
        ...
        #  过滤出：自己创建的dashboard或者别人公开的dashboard
        # 改为
        #  过滤出：自己创建的dashboard 或者 （别人公开的dashboard  && 自己拥有查看权限）
        query = query.filter(or_(Dashboard.user_id == user_id,
                                 and_(Dashboard.is_draft == False, Dashboard.id.in_(can_view_dashboard))
                                 ))
        return query

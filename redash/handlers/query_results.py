...
class QueryResultListResource(BaseResource):
    ...
    def post(self):
        ...
        :qparam string query: The query text to execute  # query字符串
        :qparam number query_id: The query object to update with the result (optional)  # query的ID
        :qparam number max_age:  表示本次query是否要立即执行，还是使用缓存，主要是通过这个age是不是过期 If query results less than `max_age` seconds old are available, return them, otherwise execute the query; if omitted, always execute
        :qparam number data_source_id: ID of data source to query  # 查询的是哪个数据源
        """
        params = request.get_json(force=True)  # 将参数json化
        # 从请求参数中提起一些特殊的参数。
